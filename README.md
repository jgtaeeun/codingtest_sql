...
|분류|함수|인수여부|설명|
|:--:|:--:|:--:|:--:|
|순위 함수|RANK|생략|동일 값은 같은 순위 부여 <br> 다음 순위는 건너뜀 <br> (1, 1, 3...)|
||DENSE_RANK|생략|동일 값은 같은 순위 부여, 다음 순위는 연속적 <br>(1, 1, 2...)|
||ROW_NUMBER|생략|동일 값이라도 고유한 일련번호 부여 <br>(1, 2, 3...)|
|집계 함수|SUM, AVG, MAX, MIN, COUNT|필수|파티션 내의 누적 합계나 평균 등을 계산|
|행 순서 함수|LAG(expr, offset, default) <br>expr: 가져올 컬럼명 (필수)<br>offset: 몇 번째 이전 행인지 (선택, 기본값 1)<br>default: 이전 행이 없을 때 채울 값 (선택)|필수|현재 행보다 이전(앞) 행의 값을 가져옴|
||LEAD|필수|현재 행보다 이후(뒤) 행의 값을 가져옴|
|비율 함수|NTILE(n)|필수|데이터를 n개의 그룹으로 등분 (예: 사분위수)|
||PERCENT_RANK|생략|해당 행의 백분위 순위를 0~1 사이로 계산|
||CUME_DIST|생략|누적 분포|
...
- PERCENT_RANK 함수
    - $$\text{PERCENT\_RANK} = \frac{\text{순위} - 1}{\text{전체 행 수} - 1}$$
    - 순위: RANK() 함수가 반환하는 값과 동일합니다.
    - 0의 의미: 가장 높은 순위(1등)는 분자가 $1 - 1 = 0$이 되어 결과값이 0이 됩니다.
    - 1의 의미: 가장 낮은 순위는 분모와 분자가 같아져 결과값이 1이 됩니다.

|학생|점수|RANK|PERCENT_RANK |계산식결과값|
|:--:|:--:|:--:|:--:|:--:|
|A|100|1|$(1-1) / (5-1)$|0.0|
|B|90|2|$(2-1) / (5-1)$|0.25|
|C|80|3|$(3-1) / (5-1)$|0.5|
...
- CUME_DIST()
    - 반환 값: 0보다 크고 1보다 작거나 같은 실수 ($0 < \text{CUME\_DIST()} \le 1$).
    - 계산 공식:$$\text{CUME\_DIST()} = \frac{\text{현재 행의 값보다 작거나 같은 행의 수}}{\text{전체 행의 수}}$$
    - 주의: 동일한 값이 있을 경우(Tie), 동일한 CUME_DIST 값을 가집니다.


|Student|Score|계산 과정 (작거나 같은 행 수 / 전체 5개)|cumulative_dist|
|:--:|:--:|:--:|:--:|
|A|60|1 / 5|0.2 (하위 20%)|
|B|70|2 / 5|0.4 (하위 40%)|
|C|80|4 / 5 <br> (본인 포함 80점이 2명이므로 4번째까지 포함)|0.8 (하위 80%)|
|D|80|4 / 5|0.8 (하위 80%)|
|E|90|5 / 5|1.0 (상위 100%)|
